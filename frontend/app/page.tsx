...
export default function Home() {
  ...
  async function loadData(address: string) {
    try {
      ...
      const formattedCollateral = ethers.formatEther(
      ...
      const formattedProtocolCollateral = ethers.formatEther(
        vaultManagerFaithBalance
      );
      const formattedProtocolDebt = ethers.formatEther(totalTfUsdSupply);

      const available =
        Number(formattedBorrowLimit) - Number(formattedDebt);
  ...
  async function loadActivity() {
    try {
      ...
      const oracle: any = new ethers.Contract(
        MOCK_ORACLE_ADDRESS,
        MOCK_ORACLE_ABI,
        provider
      );
      ...
      const [
        ...
        vault.queryFilter(
          vault.filters.CollateralDeposited(),
          DEPLOYMENT_BLOCK,
          "latest"
        ),
        vault.queryFilter(
          vault.filters.CollateralWithdrawn(),
          DEPLOYMENT_BLOCK,
          "latest"
        ),
        vault.queryFilter(
          vault.filters.TfUSDBorrowed(),
          DEPLOYMENT_BLOCK,
          "latest"
        ),
        vault.queryFilter(
          vault.filters.TfUSDRepaid(),
          DEPLOYMENT_BLOCK,
          "latest"
        ),
        vault.queryFilter(
          vault.filters.VaultLiquidated(),
          DEPLOYMENT_BLOCK,
          "latest"
        ),
        oracle.queryFilter(
      ...
      for (const event of depositEvents as any[]) {
        ...
        activityItems.push({
          ...
          type: "Deposit",
          title: "tFAITH Deposited",
          description: `${shortAddress(user)} deposited ${amount} tFAITH`,
      ...
      for (const event of withdrawEvents as any[]) {
        ...
        activityItems.push({
          id: `${event.transactionHash}-${event.index}`,
          type: "Withdraw",
          title: "tFAITH Withdrawn",
          description: `${shortAddress(user)} withdrew ${amount} tFAITH`,
          blockNumber: event.blockNumber,
          txHash: event.transactionHash,
          user,
          amount,
      ...
      for (const event of borrowEvents as any[]) {
        ...
        activityItems.push({
          id: `${event.transactionHash}-${event.index}`,
          type: "Borrow",
          title: "tfUSD Borrowed",
          description: `${shortAddress(user)} borrowed ${amount} tfUSD`,
          blockNumber: event.blockNumber,
          txHash: event.transactionHash,
          user,
          amount,
      ...
      for (const event of repayEvents as any[]) {
        ...
        activityItems.push({
          ...
          type: "Repay",
          title: "tfUSD Repaid",
          description: `${shortAddress(user)} repaid ${amount} tfUSD`,
          blockNumber: event.blockNumber,
          txHash: event.transactionHash,
          user,
          amount,
      ...
      for (const event of liquidationEvents as any[]) {
        ...
        const collateralSeized = Number(
          ethers.formatEther(event.args.collateralSeized)
        );

        activityItems.push({
          id: `${event.transactionHash}-${event.index}`,
          type: "Liquidation",
          title: "tVault Liquidated",
          description: `${shortAddress(liquidator)} liquidated ${shortAddress(
            user
          )} — ${debtRepaid} tfUSD repaid, ${collateralSeized} tFAITH seized`,
          blockNumber: event.blockNumber,
          txHash: event.transactionHash,
          user,
          liquidator,
      ...
      for (const event of oracleEvents as any[]) {
        const previousPrice = Number(
          ethers.formatEther(event.args.previousPrice)
        );
        ...
        activityItems.push({
          id: `${event.transactionHash}-${event.index}`,
          type: "Oracle",
          title: "tMockOracle Updated",
          description: `tFAITH price changed from $${previousPrice} to $${newOraclePrice}`,
          blockNumber: event.blockNumber,
          txHash: event.transactionHash,
          previousPrice,
      ...
      activityItems.sort((a, b) => b.blockNumber - a.blockNumber);
  ...
  useEffect(() => {
    if (wallet) {
      refreshEverything(wallet);
    }
  }, [wallet]);

  return (
    <main className="min-h-screen bg-[#050608] p-8 text-white">
      <section className="mb-8 flex flex-col justify-between gap-6 rounded-3xl border border-white/10 bg-white/[0.03] p-8 lg:flex-row lg:items-center">
        <div>
          ...
          <p className="mt-3 max-w-2xl text-lg text-zinc-400">
            Real-time collateralized credit on MegaETH with tFAITH collateral,
            tfUSD borrowing, oracle-based risk simulation, and liquidations.
          </p>
        </div>
        ...
        {!wallet ? (
          <button
            ...
            className="rounded-2xl bg-blue-600 px-8 py-4 text-lg font-bold transition hover:bg-blue-500"
          >
            Connect Wallet
          </button>
        ) : (
      ...
      <section className="mb-8 rounded-3xl border border-white/10 bg-white/[0.03] p-6">
        <h2 className="text-2xl font-bold">
          Why FAITH is built for MegaETH
        </h2>

        <div className="mt-5 grid gap-5 md:grid-cols-3">
          <NarrativeCard
            title="Real-Time Risk"
            body="FAITH reacts instantly to collateral price shocks and displays borrower solvency live."
          />

          <NarrativeCard
            title="Fast Liquidation"
            body="Unsafe debt positions can be cleared immediately, reducing stale-state risk in credit markets."
          />

          <NarrativeCard
            title="Visible On-Chain Activity"
            body="Every deposit, borrow, price update, and liquidation is exposed through a transparent activity layer."
          />
        </div>
      </section>
      ...
      <section className="mb-8">
        <div className="mb-4 flex items-center justify-between">
          ...
          <div
            className={`rounded-full border px-4 py-2 text-sm font-bold ${riskStatus.bg} ${riskStatus.border} ${riskStatus.color}`}
          >
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
          <MetricCard
            label="Total tFAITH Collateral"
            value={Number(protocolCollateral).toLocaleString()}
            helper="Held inside tVaultManager"
          />

          <MetricCard
            label="Total tfUSD Debt Supply"
            value={Number(protocolDebtSupply).toLocaleString()}
            helper="Outstanding test credit"
          />

          <MetricCard
            label="tFAITH Oracle Price"
            value={`$${oraclePrice}`}
            helper="tMockOracle live value"
          />

          <MetricCard
            label="tVault Address"
            value={`${VAULT_MANAGER_ADDRESS.slice(0, 6)}...${VAULT_MANAGER_ADDRESS.slice(-4)}`}
            helper="Current test deployment"
          />
        </div>
      </section>
      ...
      <section className="mb-8 rounded-3xl border border-white/10 bg-white/[0.03] p-6">
        <div className="mb-5 flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
          <div>
            ...
            <p className="mt-1 text-zinc-400">
              Live account position and borrowing health.
            </p>
          </div>
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-7">
          <MetricCard
            label="tFAITH Balance"
            value={Number(faithBalance).toLocaleString()}
          />

          <MetricCard
            label="tfUSD Balance"
            value={Number(fusdBalance).toLocaleString()}
          />
      ...
      <section className="mb-8">
        ...
        <div className="grid gap-6 lg:grid-cols-2 xl:grid-cols-3">
          <ActionCard
            ...
            placeholder="Amount"
            buttonLabel="Deposit tFAITH"
            buttonClassName="bg-green-600 hover:bg-green-500"
            onClick={depositCollateral}
          />

          <ActionCard
            title="Borrow tfUSD"
            description="Mint tfUSD against available tFAITH collateral."
            inputValue={borrowAmount}
            onInputChange={setBorrowAmount}
            placeholder="Amount"
            buttonLabel="Borrow tfUSD"
          ...
          <ActionCard
            title="Repay tfUSD"
            description="Repay test debt and restore vault health."
            inputValue={repayAmount}
            onInputChange={setRepayAmount}
            placeholder="Amount"
            buttonLabel="Repay tfUSD"
            buttonClassName="bg-yellow-600 hover:bg-yellow-500"
            onClick={repayTfUSD}
          />

          <ActionCard
            title="Withdraw tFAITH"
            description="Withdraw collateral while preserving solvency."
            inputValue={withdrawAmount}
            onInputChange={setWithdrawAmount}
            placeholder="Amount"
            buttonLabel="Withdraw tFAITH"
            buttonClassName="bg-red-600 hover:bg-red-500"
            onClick={withdrawCollateral}
          />

          <ActionCard
            title="Liquidate tVault"
            description="Liquidate an unsafe vault using tfUSD."
            inputValue={liquidateAddress}
            onInputChange={setLiquidateAddress}
            placeholder="User wallet address"
            buttonLabel="Liquidate tVault"
            buttonClassName="bg-rose-700 hover:bg-rose-600"
            onClick={liquidateVault}
          />
          ...
          <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-6">
            ...
            <p className="mt-2 min-h-[48px] text-sm text-zinc-400">
              Simulate tFAITH price movement and test liquidation behavior.
            </p>

            <input
              value={newPrice}
              onChange={(e) => setNewPrice(e.target.value)}
              placeholder="New tFAITH price, ex: 0.4"
              className="mt-5 w-full rounded-2xl border border-white/10 bg-black/30 p-4 outline-none placeholder:text-zinc-600 focus:border-purple-400/70"
            />

            <div className="mt-4 grid gap-3">
              <button
                onClick={() => setOraclePriceOnchain(newPrice)}
                className="w-full rounded-2xl bg-purple-600 p-4 font-bold transition hover:bg-purple-500"
              >
                Set tFAITH Price
              </button>

              <button
                onClick={() => setOraclePriceOnchain("0.4")}
                className="w-full rounded-2xl bg-orange-600 p-4 font-bold transition hover:bg-orange-500"
              >
                Crash tFAITH to $0.40
              </button>

              <button
                onClick={() => setOraclePriceOnchain("1")}
                className="w-full rounded-2xl bg-zinc-700 p-4 font-bold transition hover:bg-zinc-600"
              >
                Reset tFAITH to $1.00
              </button>
            </div>
          </div>
        </div>
      ...
      <section className="mb-8 rounded-3xl border border-white/10 bg-white/[0.03] p-6">
        <div className="mb-5 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
          <div>
            ...
            <p className="mt-1 text-zinc-400">
              Live events emitted from tVaultManager and tMockOracle.
            </p>
          </div>

          <button
            onClick={loadActivity}
            className="rounded-2xl border border-white/10 bg-black/30 px-5 py-3 font-semibold transition hover:bg-white/10"
          >
        ...
        {activity.length === 0 ? (
          <div className="rounded-2xl border border-white/10 bg-black/30 p-6 text-zinc-400">
            No protocol activity found yet.
          </div>
        ) : (
      ...
      <section className="rounded-3xl border border-white/10 bg-white/[0.03] p-6">
        <h2 className="text-2xl font-bold">
          Current Testnet Deployment Registry
        </h2>
      ...
      {status && (
        <div className="mt-8 rounded-3xl border border-white/10 bg-white/[0.03] p-5 text-lg">
          {status}
        </div>
      )}
    </main>
...
function MetricCard({
  ...
  return (
    <div className="rounded-3xl border border-white/10 bg-black/30 p-5">
      ...
      <p className={`mt-3 break-words text-2xl font-bold ${valueClassName}`}>
        {value}
      </p>
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <p className="text-sm font-bold text-zinc-500">{number}</p>
        <div
  ...
  return (
    <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-6">
      ...
      <input
        ...
        className="mt-5 w-full rounded-2xl border border-white/10 bg-black/30 p-4 outline-none placeholder:text-zinc-600 focus:border-blue-400/70"
      />

      <button
        onClick={onClick}
        className={`mt-4 w-full rounded-2xl p-4 font-bold transition ${buttonClassName}`}
      >
        {buttonLabel}
      </button>
    </div>
...
function ActivityRow({
  ...
  return (
    <div className="flex flex-col justify-between gap-4 rounded-2xl border border-white/10 bg-black/30 p-5 lg:flex-row lg:items-center">
      <div className="flex items-start gap-4">
        <div
          className={`rounded-full border px-3 py-1 text-xs font-bold ${badgeStyle}`}
        >
          {item.type}
        </div>
      ...
      <div className="text-sm text-zinc-500">
        ...
        <p className="font-mono">{shortHash(item.txHash)}</p>
      </div>
    </div>
  );
}