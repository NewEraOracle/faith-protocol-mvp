...
async function safeQueryFilter(contract: any, eventFilter: any, fromBlock?: any, toBlock?: any) {
  try {
    ...
    console.warn("Faith Monetary Protocol dashboard activity filter skipped:", error);
    return [];
  }
}
...
export default function Home() {
  ...
  const healthNumber = healthFactor === "8" ? Number.POSITIVE_INFINITY : Number(healthFactor);
  ...
  const pcsRisk = useMemo(() => {
    return calculatePCSRisk({
      ...
      healthFactor: healthFactor === "8" ? null : Number(healthFactor),
  ...
  const recommendedAction = useMemo(() => {
    ...
    return "Demo completed successfully Ã¢Å“â€";
  }, [wallet, demoProgress]);
  ...
  async function loadData(address: string, providerOverride?: any) {
    try {
      ...
      setHealthFactor(healthRaw === ethers.MaxUint256 ? "8" : (Number(healthRaw) / 100).toFixed(2));
    } catch (error) {
      console.error(error);
      setStatus("Loading protocol data failed Ã¢ÂÅ’");
    }
  }
  ...
  async function loadActivity(providerOverride?: any) {
    try {
      ...
      for (const event of liquidationEvents as any[]) {
        ...
        items.push({ id: `${event.transactionHash}-${event.index}`, type: "Liquidation", title: "Liquidation Executed", description: `${shortAddress(liquidator)} cleared unsafe debt for ${shortAddress(user)} Ã¢â‚¬â€ ${debtRepaid} tfUSD repaid, ${collateralSeized} FXMP collateral seized`, blockNumber: event.blockNumber, txHash: event.transactionHash, user, liquidator, debtRepaid, collateralSeized });
      }
      ...
      console.error(error);
      setStatus("Loading on-chain activity failed Ã¢ÂÅ’");
    } finally {
  ...
  async function resetDemoFlow() {
    ...
    setStatus("Demo Flow reset to 0/5 Ã¢Å“â€");
  }
  ...
  async function depositCollateral() {
    try {
      ...
      setStatus("FXMP deposit successful Ã¢Å“â€");
      setDepositAmount("");
      await refreshEverything(wallet);
    } catch (error) {
      console.error(error);
      setStatus("FXMP deposit failed Ã¢ÂÅ’");
    }
  }
  ...
  async function borrowFUSD() {
    try {
      ...
      console.error(error);
      setStatus("FUSD borrow failed Ã¢ÂÅ’");
    }
  }
  ...
  async function repayFUSD() {
    try {
      ...
      console.error(error);
      setStatus("FUSD repayment failed Ã¢ÂÅ’");
    }
  }
  ...
  async function withdrawCollateral() {
    try {
      ...
      await tx.wait();
      setStatus("FXMP withdrawal successful Ã¢Å“â€");
      setWithdrawAmount("");
      await refreshEverything(wallet);
    } catch (error) {
      console.error(error);
      setStatus("FXMP withdrawal failed Ã¢ÂÅ’");
    }
  }
  ...
  async function liquidateVault() {
    try {
      ...
      if (targetDebt.toString() === "0") {
        setStatus("Target tVault has no FUSD debt Ã¢ÂÅ’");
        return;
      }
      ...
      setStatus("tVault liquidation successful Ã¢Å“â€");
      setLiquidateAddress("");
      await refreshEverything(wallet);
    } catch (error) {
      console.error(error);
      setStatus("tVault liquidation failed Ã¢ÂÅ’");
    }
  }
  ...
  async function setOraclePriceOnchain(price: string) {
    try {
      ...
      setStatus("tMockOracle price updated Ã¢Å“â€");
      setNewPrice("");
      await refreshEverything(wallet);
    } catch (error) {
      console.error(error);
      setStatus("tMockOracle update failed Ã¢ÂÅ’");
    }
  }
  ...
  return (
    <main className="faith-dashboard min-h-screen p-6 text-white lg:p-8">
      ...
      <section className="mb-8 rounded-3xl border border-cyan-500/20 bg-cyan-500/[0.06] p-6">
        <div className="mb-6 flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
          <div>
            ...
            <h2 className="text-3xl font-bold">Demo Flow: Prove the Faith Monetary Protocol Credit, Treasury, and PCS Risk Loop</h2>
      ...
      <section className="mb-8 rounded-3xl border border-amber-300/20 bg-gradient-to-br from-amber-300/[0.10] via-white/[0.035] to-cyan-300/[0.06] p-6 shadow-[0_0_70px_rgba(251,191,36,0.08)]">
        <div className="mb-6 flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
          ...
          <div className={`rounded-full border px-5 py-3 text-sm font-black ${
            ...
            {pcsRisk.pcsRiskLevel} Â· {pcsRisk.pcsRiskScore}/100
          </div>
        </div>
      ...
      <section className="mb-8 rounded-3xl border border-white/10 bg-white/[0.03] p-6">
        <h2 className="text-2xl font-bold">Why Faith Monetary Protocol is built for MegaETH</h2>
        <div className="mt-5 grid gap-5 md:grid-cols-3">
          <NarrativeCard title="Real-Time Risk" body="Faith Monetary Protocol reacts instantly to collateral price shocks and displays borrower solvency live." />
      ...
      <section className="mb-8 rounded-3xl border border-white/10 bg-white/[0.03] p-6">
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
          ...
          <div className="rounded-3xl border border-white/10 bg-black/30 p-5">
            ...
            <p className={`mt-3 break-words text-2xl font-bold ${healthFactor !== "8" && Number(healthFactor) < 1.1 ? "text-red-400" : healthFactor !== "8" && Number(healthFactor) < 1.5 ? "text-orange-300" : "text-green-400"}`}>{healthFactor}</p>
            <div className="mt-4">
              <div className="h-3 w-full overflow-hidden rounded-full bg-white/10">
                <div className={`h-full transition-all duration-700 ${healthFactor !== "8" && Number(healthFactor) < 1.1 ? "bg-red-500" : healthFactor !== "8" && Number(healthFactor) < 1.5 ? "bg-orange-400" : "bg-emerald-400"}`} style={{ width: healthFactor === "8" ? "100%" : `${Math.min(100, Number(healthFactor) * 50)}%` }} />
              </div>
      ...
      <section className="mb-8 rounded-3xl border border-white/10 bg-white/[0.03] p-6">
        <div className="mb-6">
          ...
          <p className="mt-2 max-w-4xl text-zinc-400">
            Faith Monetary Protocol connects human-built productive value to a machine-regulated financial layer on MegaETH.
            Humans build the world. Technology regulates the economy. Faith Monetary Protocol connects both layers through credit,
        ...
        <div className="grid gap-5 md:grid-cols-3">
          ...
          <div className="rounded-3xl border border-cyan-400/20 bg-cyan-400/10 p-5 shadow-[0_0_45px_rgba(34,211,238,0.08)]">
            ...
            <p className="mt-5 text-xs font-bold uppercase tracking-[0.18em] text-cyan-100">
              Faith Monetary Protocol transforms value into credit
            </p>
          </div>
      ...
      <section className="mb-8">
        ...
        <div className="grid gap-6 lg:grid-cols-2 xl:grid-cols-3">
          ...
          <ActionCard title="Borrow tfUSD" description="Mint tfUSD against available FXMP collateral." inputValue={borrowAmount} onInputChange={setBorrowAmount} placeholder="Amount" buttonLabel="Borrow tfUSD" buttonClassName="bg-blue-600 hover:bg-blue-500" onClick={borrowFUSD} />
