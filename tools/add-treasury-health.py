...
# Make the protocol overview grid support 6 cards
s = s.replace(
...
marker = '''          <MetricCard label="Total tFAITH Collateral" value={Number(protocolCollateral).toLocaleString()} helper="Held inside tVaultManager" />'''

treasury_card = '''          <div className="rounded-3xl border border-emerald-400/20 bg-emerald-400/10 p-5 shadow-[0_0_45px_rgba(16,185,129,0.08)]">
...
if "Treasury Health" in s:
    print("Treasury Health already exists. No changes made.")
elif marker not in s:
    print("Metric marker not found. No changes made.")
else:
    s = s.replace(marker, treasury_card + marker, 1)
    path.write_text(s, encoding="utf-8")
    print("Treasury Health card added successfully.")
